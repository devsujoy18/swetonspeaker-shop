...
class TypeProductComponent extends Component
{
    ...
    public function getProductsProperty()
    {
        ...
        $categories = Category::with([
            'products' => function ($q) {
                ...
                if (!empty($this->selectedOhms) && $this->selectedOhms !== 'All Ohms') {
                    $q->whereHas('combinations', fn($q2) =>
                        $q2->where('name', $this->selectedOhms)
                    );
                }

                if (!empty($this->search)) {
                    $q->where(fn($q2) =>
                        $q2->where('products.name', 'like', "%{$this->search}%")
        ...
        foreach ($categories as $category) {
            $category->products = $category->products->map(function ($product) {
                $product->effective_price = $product->priceAttributes->min('price') ?? $product->price;
                return $product;
            });
    ...
    public function addTocart($productId, $priceAttributeId = null){
        try {
            ...
            if ($priceAttributeId) {
                $attribute = $product->priceAttributes->firstWhere('id', $priceAttributeId);
                if (!$attribute) {
            ...
            if($product){
                ...
                Cart::add([
                    'id' => $cartId,
                    'name' => $product->name . ($attributeName ? ' (' . $attributeName . ')' : ''),
                    'price' => $price,
                    'quantity' => 1,
                    'attributes' => [
                        'mrp' => $mrp,
